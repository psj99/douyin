...
// 视频流
func VideoFeed(ctx context.Context, req *request.VideoFeedReq) (resp *response.VideoFeedResp, err error) {
	// 获取请求用户ID 默认为0(不存在)
	var me_id uint = 0
	if req.Token != "" {
		// 解析/校验token (自动验证有效期等)
	...
	for _, video := range videos {
		...
		// 检查是否点赞
		is_favorite, err := dao.CheckFavorite(context.TODO(), me_id, video.ID)
		if err == nil {
			if is_favorite {
		...
		if me_id != 0 { // 若登录则检查是否关注
			is_follower, err := dao.CheckFollower(context.TODO(), me_id, user.ID)
			if err == nil {
				if is_follower {
